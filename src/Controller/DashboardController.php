  ...
  public function mainDashboardCallback() {
    ...
    try {
      ...
      foreach ($first_ten_checkout_books as $rid => $recent_checkout_book) {
        ...
        $teacher = user_load($recent_checkout_book['teacher']);
        $student = user_load($recent_checkout_book['student']);
        ...
        $first_ten_checkout_books[$rid]['check_out_date'] = format_date($recent_checkout_book['check_out_date'], 'custom', 'd/m/Y - H:s');;
